...
Companion to [`regression-runbooks`](../regression-runbooks/README.md): runbooks are the **standing** coverage ("this area still works"), a smoke test is the **delta** ("this change works, and here's what to poke"). This plugin copies the steps of a covering runbook case into the smoke test — self-contained, so the tester never navigates away — marks where they came from, and pushes permanent behaviour back into the runbook.
...
3. **Read the runbooks** — copy in the click path of a case that already walks the journey (marked `*(from TC-…)*`); flag one that ought to exist.
