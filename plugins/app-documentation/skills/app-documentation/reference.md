...
- **For shared infrastructure, grep for *reads*, not registrations.** A registry, bus or manager that every part dutifully registers with can have **zero readers**, and nothing distinguishes that from a live dependency at the call site. One run found a cache manager with eight `registerService` calls and no caller of any of its ten read methods, while two docs had already been drafted claiming it powered the refresh UI. The false confidence came from three sources agreeing: the registration code was real, the class had a complete API, and the repo's own instruction file asserted the connection. **A project instruction file (`CLAUDE.md`, `AGENTS.md`) is source 5 like any other doc**, and it feels authoritative in a way a stale `docs/` page does not. Before writing "X powers Y", grep for calls to X's read methods. Write-only infrastructure is worth documenting as such: it tells the next developer that changing it cannot break the UI.
- **A grep-based label gate cannot see a composed label.** Gate 8 greps each bolded control label in the UI source, and in any app that assembles labels inline (`{env} ENVIRONMENT`, `` `Cached ${age}` ``, `Verify all {changed ? "imported" : "CSV"} values`) it reports every one as a miss while the guide is correct. One run produced 16 candidates of which none was real drift. Expect a high false-positive rate, verify each hit against the composing expression, and when writing the guide prefer quoting the invariant part of a composed label or stating the variants, since a label containing a ternary has more than one true value.
- **Read the enforcement site, not just the declaration, for every limit.** A limit can be declared, passed by callers, and never read: one run found a 10 MB file-size cap accepted as a validation parameter and never referenced (the tell was an `eslint-disable` for an unused parameter), and a row cap that raised a *warning* while the proceed check tested only for errors. Both were fiction, and a doc quoting the declaration would have reported two constraints the app does not apply. Quote a limit only after seeing the branch that acts on it, and where a limit warns rather than blocks, say so in the guide: "too large" and "large and slow" are different sentences for the reader.
- **Enumerate which members of a constrained set are *produced*, not just declared.** The declaration is authoritative for what values exist and silent on which the app ever emits. One run found a four-value `Severity` enum with two members produced by 23 mapping definitions and two produced by nothing anywhere in the tree; a grep for the two names settled it in seconds. A Reference table copied straight from the enum would have told the reader to expect four, and a client author would have written two branches that never execute. Same discipline as the limit above, one extra column: per member, grep for its producers and record the count. A zero belongs in prose as well as the table, because "declared but never produced" is an absence no reader can infer from a complete-looking list.
- **A planning document or defects register written in the present tense may describe a branch you are not on.** Discovery already ranks internal notes last for facts, but the sharper failure is a register that is accurate *about a different ref*. One run met an entry marked "Fixed, PR A" that stated a global result filter "starts working for that endpoint too" once the fix landed; PR A was not on the working branch, so the sentence was false there and the file gave no way to tell. The tell is any "once X lands, Y works" or "after Task N it does Z" phrasing in a document that also carries a *Fixed* or *Done* status, since that status refers to somewhere else. Open the file the claim names and read the line before repeating it. When a claim turns out to be branch-dependent, say which side of the change the doc describes rather than dropping the sentence: the reader is standing on a specific branch too.
